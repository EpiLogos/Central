...
| `central.world.here` | resolve a working directory (default: the process cwd) or a named Work member to the Local World (`control:root`) and its Project World: register roots at both scopes, the civil-time policy ref and revision, declared Workcells with their root NOW state, and whether the authored world record matches the scope identity. A registered development worktree resolves through its `repository` grant; absence and ambiguity are data (`central.world-here/v1`). Read-only | `action run central.world.here '{"cwd":"<path>"}'` |
| `central.position.list` | list the World Position definitions (`central.world-position/v1`, `{Control\|ProjectCentral}/relations/positions/<slug>.json`) of the root or one Project, with root Positions as `inherited`; files failing strict validation are listed in `invalid[]`. Uncapped, read-only | `action run central.position.list '{"project":"<member>"}'` |
| `central.position.read` | read one Position by `central:position:<world_ref>:<slug>` with its source ref and revision; refuses with fact, consequence and next command (`central.position_not_found`, `central.position_invalid`). Read-only | `action run central.position.read '{"position_ref":"<ref>"}'` |
...
| `projectcentral.source.transfer.export` | export an explicitly scoped, explicitly directed source transfer bundle for another ground of the same world (world-relative paths and content revisions only); omit `project` for the root register (`control:root`), where agents carry the root agent ground but never `Control/user` and nobody carries `Control/machines` — see `docs/SOURCE-TRANSFER.md` | `action run projectcentral.source.transfer.export` |
| `projectcentral.source.transfer.apply` | apply a transfer bundle: fast-forward from the recorded base, already-present, or an explicit recorded conflict - divergent source is never overwritten, transfers never delete; at the root register every creation needs `accept_unestablished_lineage` and an unverifiable identity needs `accept_unestablished_identity` | `action run projectcentral.source.transfer.apply` |
