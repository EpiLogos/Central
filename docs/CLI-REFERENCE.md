...
| `projectcentral.now.return` | write one attributed Agent handoff/question/note/learning (`central.project-now.handoff/v1`) into the register's NOW agents area. With `project`, the return lands in that project's `ProjectCentral/now`; omit `project` for root scope (`Control/agents/now/agents`, created on write when absent). External Run/Session/Focus/source identities remain refs rather than being duplicated | `action run projectcentral.now.return` |
...
| `agent-profile.review` | read-only exact-source review of one AgentProfile. `scope` is required; `project` is optional; `profile_ref` is required | `action run agent-profile.review` |
| `agent-profile.roster` | read-only native reusable Agent roster. `scope` is required; `project` is optional | `action run agent-profile.roster` |
| `agent-profile.accept` | authenticated human acceptance of one exact reviewed AgentProfile revision. `scope`, `profile_ref`, `expected_revision` and `expected_content_digest` are required; `expected_authority_revision` is optional. Uses the existing native-action-authority credential via the protected process channel; never changes generated provenance; never grants execution, model, tool or child-Agent authority; a JSON actor label cannot manufacture acceptance | `action run agent-profile.accept` |
