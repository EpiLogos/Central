...
AIKit must preserve source eligibility/treatment when resolving guidance.
...
4. relation adoption changes no source bytes/path and no operational precedence;
5. `.no-agent-retrieval` excludes marked source from the stock Agent read model;
6. Central states AIKit as operational resolution owner rather than inventing its own precedence;
7. observed interaction can create proposal pressure but not automatic governance mutation;
8. pruning and Skill extraction remain normal maintenance outcomes;
9. Project meaning and Agent-governance source remain distinct but mutually legible.
