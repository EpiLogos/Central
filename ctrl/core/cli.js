...
function parseArguments(argv) {
  ...
  let commandKey;
  if (positional[0] === "action" && positional[1] === "list" && positional.length === 2) {
    commandKey = "action.list";
  } else if (positional.length === 1) {
    commandKey = positional[0];
  } else if (positional.length === 0) {
    return { structured, error: "An Action or command is required." };
  } else {
...
export async function runCli(argv, { env = process.env, home, cwd = process.cwd() } = {}) {
  ...
  const result = await registry.execute(parsed.actionId, {}, {
    rootOptions: {
