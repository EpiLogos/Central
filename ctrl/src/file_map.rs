...
fn resolve(all: &[Scope], input: &Value) -> io::Result<Value> {
...
fn inspect(all: &[Scope], input: &Value) -> io::Result<Value> {
    ...
    let available = version.is_ok() && backend.present();
    Ok(
        json!({"world_ref":scope.world,"revision":scope.basis()?,"scopes":all.iter().map(|s|json!({"world_ref":s.world,"project":s.project,"path":s.root})).collect::<Vec<_>>(),"resources":entries(scope)?,"links":scope.ground()?.links,"database":backend.db(),"provider":{"available":available,"version":version.as_ref().ok().map(|v|v.trim()),"tested_version":native::VERSION,"fulltext":available,"hybrid":available && index.embeddings,"semantic":false,"reason":version.err().map(|e|e.to_string())}}),
    )
}
...
pub fn execute(root: &Path, op: &str, input: &Value) -> io::Result<Value> {
    ...
    let _lock = if matches!(
        op,
        "inspect" | "search" | "resolve" | "locate" | "projection-read"
    ) {
        None
    } else {
