        ...
        if let (Some(old), Some(row)) = (&existing, &row) {
            ...
            if old.revision != entry.revision || old.url != url || index.embeddings != embeddings {
                ...
                if current_desc != old.generated_description {
                    return Err(conflict("Managed indexed description was edited in bkmr; preserve/reconcile it before refresh"));
                }
                let mut args = vec![
                    "update".into(),
                    record_id.to_string(),
            ...
            if let Some(row) = matches.first() {
                ...
                if records.iter().any(|r| native::record(r)["url"] == url) {
                    return Err(conflict("Existing unowned bookmark for this URI must be explicitly adopted, not overwritten"));
                }
                let mut args = vec![
                    "add".into(),
                    url.clone(),
