...
fn submit(scope: &Scope, input: &Value, principal: &Principal, now: u64) -> io::Result<Value> {
    ...
    let record = Received {
        ...
        document_id: text(&document, "document_id")?.into(),
        proposed_source_revision: expected.into(),
        proposal,
        ...
        status: if stale { "needs-review" } else { "pending" }.into(),
        stale_at_arrival: stale,
        review: None,
...
fn review(scope: &Scope, input: &Value, principal: &Principal, now: u64) -> io::Result<Value> {
    ...
    if !matches!(disposition, "accepted" | "rejected") {
        return Err(invalid(
            "review disposition is accepted or rejected; inclusion is a separate Action",
        ));
    }
    let source_revision = if disposition == "accepted" {
        ...
        record.proposed_source_revision.clone()
    };
...
fn include(
    ...
    let request = if recover {
        ...
        let current = scope.read(&record.source_ref)?;
        if current.revision.revision != review.source_revision
...
fn list(scope: &Scope, input: &Value) -> io::Result<Value> {
    ...
    let entries = match fs::read_dir(scope.root.join(AREA)) {
        ...
        Err(error) if error.kind() == io::ErrorKind::NotFound => {
            return Ok(
                json!({"schema":"central.receiving-page/v1","scope_ref":scope.world_ref,"returns":[],"more":false,"next_after":null}),
            )
        }
        Err(error) => return Err(error),
    };
    ...
    for entry in entries {
        ...
        if scope.read(&record.source_ref).is_err() {
            withheld += 1;
            continue;
        }
    ...
    Ok(
        json!({"schema":"central.receiving-page/v1","scope_ref":scope.world_ref,"returns":records,"more":more,"next_after":next,"withheld_unavailable_sources":withheld,"proposal_bodies_in_page":false}),
    )
}
