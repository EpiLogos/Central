...
fn key(value: &str) -> String {
    source::revision(value).replace([':', '/'], "_")
}
fn read_json(scope: &Scope, path: &str) -> io::Result<Value> {
    ...
    if !file.metadata()?.is_file() || file.metadata()?.len() > bound {
        return Err(invalid("history record is not bounded regular data"));
    }
...
pub(crate) fn replace(
    ...
    if scope.read(&reading.source.source_ref)?.revision != reading.revision {
        return Err(conflict("source changed before its history commit"));
    }
    if content == reading.content {
        return Ok(reading.clone());
    }
    snapshot(scope, &area, &reading.content)?;
    snapshot(scope, &area, content)?;
    let cursor = events(scope, &area)?
        .last()
        .and_then(|v| v["cursor"].as_u64())
        .unwrap_or(0)
        .checked_add(1)
...
pub(crate) fn read(scope: &Scope, input: &Value) -> io::Result<Value> {
    ...
    let current = scope.read(source_ref)?;
    let area = format!(".central/file-history/{}", key(source_ref));
    if !scope.root.join(&area).exists() {
        return Ok(
            json!({"source":current.source,"revision":current.revision,"entries":[],"more":false,"next_before":null}),
        );
    }
    let limit = input.get("limit").and_then(Value::as_u64).unwrap_or(20);
    if limit == 0 || limit > 200 {
        return Err(invalid("history limit must be 1..200"));
    }
    let before = input
        .get("before")
        .and_then(Value::as_u64)
        .unwrap_or(u64::MAX);
    let mut entries: Vec<_> = events(scope, &area)?
        .into_iter()
        .rev()
        .filter(|v| v["cursor"].as_u64().is_some_and(|n| n < before))
        .collect();
    let more = entries.len() > limit as usize;
    entries.truncate(limit as usize);
    let next = if more {
        entries.last().map(|v| v["cursor"].clone())
    } else {
        None
    };
    Ok(
        json!({"schema":"central.source-history/v1","source":current.source,"revision":current.revision,"entries":entries,"more":more,"next_before":next,"provider":"central.native-file-history","automatic_agent_or_model_invocation":false}),
    )
}
