...
use super::{
    authority::Principal,
    history, placement,
    source::{self, conflict, denied, encoded, invalid, text, Scope, SourceReading},
    temporal,
};
...
fn optional_text<'a>(input: &'a Value, key: &str) -> io::Result<Option<&'a str>> {
    match input.get(key) {
        ...
        Some(Value::String(value)) if value.len() <= 4096 => Ok(Some(value)),
        _ => Err(invalid(format!("{key} must be bounded text or absent"))),
    }
}
fn clean(content: &str) -> io::Result<String> {
    if content.len() > 256 * 1024 || content.contains('\0') {
        return Err(invalid(
            "a contribution must be bounded UTF-8 text without NUL",
        ));
    }
...
fn parse(reading: &SourceReading) -> io::Result<Value> {
    ...
    for key in ["fields", "entries", "contributions", "operations"] {
        if value[key].as_array().is_none() {
            return Err(invalid(format!("native document lacks its {key} array")));
        }
    }
    ...
    for collection in ["fields", "entries", "contributions"] {
        for entry in value[collection].as_array().unwrap() {
            ...
            if !ids.insert((collection, id)) {
                return Err(invalid(
                    "duplicate document-local identity requires explicit reconciliation",
                ));
            }
        }
    }
...
fn relation(scope: &Scope, reference: &str) -> io::Result<Value> {
    ...
    relations["relations"]
        ...
        .cloned()
        .ok_or_else(|| invalid("document requires an explicit source relation"))
}
fn metadata(scope: &Scope, reading: &SourceReading, document: &Value) -> io::Result<Value> {
    ...
    if meta["document_id"] != document["document_id"] || meta["kind"] != document["kind"] {
        return Err(conflict(
            "document identity differs from its native source relation",
        ));
    }
    Ok(meta.clone())
}
fn retain_metadata(
    ...
    let entry = relations["relations"]
        ...
        .ok_or_else(|| invalid("document source relation disappeared"))?;
    if entry["path"] != reading.source.path {
        return Err(conflict("document placement changed during mutation"));
    }
    if let Some(previous) = previous_revision {
        let recorded = entry["document"]["last_native_revision"].as_str();
        if recorded != Some(previous) && recorded != Some(reading.revision.revision.as_str()) {
            return Err(conflict(
                "document metadata changed during mutation recovery",
            ));
        }
    } else if entry.get("document").is_some()
    ...
    let roles = entry["roles"]
        .as_array_mut()
        .ok_or_else(|| invalid("source roles must be an array"))?;
    if !roles.iter().any(|role| role == ROLE) {
...
pub(crate) fn reading(scope: &Scope, input: &Value) -> io::Result<(SourceReading, Value, Value)> {
    ...
    if let Some(id) = input.get("document_id") {
        if id != &document["document_id"] {
            return Err(conflict("DocumentId does not match SourceRef"));
        }
    }
...
fn fields(input: &Value, payload: &Value) -> io::Result<Vec<Value>> {
    let fields = input
        ...
        .unwrap_or_default();
    if fields.len() > 128 {
        return Err(invalid("at most 128 explicit document fields"));
    }
    let mut ids = BTreeSet::new();
    for field in &fields {
        if !ids.insert(text(field, "id")?) {
            return Err(invalid("duplicate native field id"));
        }
        text(field, "label")?;
        if let Some(pointer) = optional_text(field, "template_pointer")? {
            if payload.pointer(pointer).is_none() {
                return Err(invalid("field mapping names a missing actual template payload key; no key is manufactured"));
            }
        }
    }
    Ok(fields)
...
pub fn create(scope: &Scope, input: &Value, principal: &Principal, now: u64) -> io::Result<Value> {
    ...
    if !matches!(kind, "day" | "flow" | "dialogue") {
        return Err(invalid("document kind is day, flow or dialogue"));
    }
    if kind == "day" {
    ...
    for binding in scope.bindings()? {
        if binding.roles.iter().any(|role| role == ROLE) {
            ...
            if document["document_id"] == id {
                if document["creation_digest"] != digest {
                    return Err(conflict("DocumentId already has another creation intent"));
                }
                return read(
                    scope,
                    &json!({"source_ref":binding.source_ref,"document_id":id}),
                );
            }
        }
    }
    ...
    let source = if kind == "day" {
        ...
        let current = scope.read(
            day["source"]["ref"]
                .as_str()
                .ok_or_else(|| invalid("Day source identity missing"))?,
        )?;
        if current.revision.revision != text(input, "expected_revision")? {
            return Err(conflict(
                "Day changed before native document initialisation",
            ));
        }
        if !current.content.is_empty() {
            return Err(denied("non-empty human Day bytes are not implicitly converted; retain/import them with explicit reviewed source migration"));
        }
        ...
        match crate::source_safety::read(&scope.root, &path) {
            Ok(raw) => {
                ...
                if existing["schema"] != SCHEMA
                    ...
                    || existing["creation_digest"] != digest
                {
                    return Err(conflict(
                        "unbound document path has different bytes/creation identity",
                    ));
                }
                document = existing;
            }
...
fn array_mut<'a>(document: &'a mut Value, key: &str) -> io::Result<&'a mut Vec<Value>> {
    document[key]
        .as_array_mut()
        .ok_or_else(|| invalid("invalid document collection"))
}
fn append(
    document: &mut Value,
    input: &Value,
    ...
    if document["contributions"]
        ...
        .any(|c| c["id"] == id)
    {
        return Err(conflict("ContributionId already exists"));
    }
    let html = clean(
        input
            .get("html")
            .and_then(Value::as_str)
            .ok_or_else(|| invalid("contribution html required"))?,
    )?;
...
fn edit(
    ...
    if document["kind"] == "day" && !human && !reviewed {
        return Err(denied("human Day contributions require explicit receiving review; direct Agent mutation is not adoption"));
    }
    if document["lifecycle"] != "open" {
        return Err(denied(
            "document is closed; reopen it explicitly before contributing",
        ));
    }
    match text(input, "operation")? {
        "entry.add" => {
            ...
            if document["entries"]
                ...
                .any(|e| e["id"] == id)
            {
                return Err(conflict("EntryId already exists"));
            }
            let reply = optional_text(input, "reply_to")?;
            if reply.is_some_and(|reference| {
                !document["entries"]
                    .as_array()
                    .unwrap()
                    .iter()
                    .any(|e| e["id"] == reference)
            }) {
                return Err(invalid(
        ...
        "entry.append" => {
            ...
            if !document["entries"]
                ...
                .any(|e| e["id"] == id)
            {
                return Err(invalid("EntryId is not in this document"));
            }
            append(
                document,
                input,
        ...
        "field.append" => {
            ...
            if !document["fields"]
                ...
                .any(|e| e["id"] == id)
            {
                return Err(invalid("field_id is not an actual supplied document field"));
            }
            append(
                document,
                input,
        ...
        "contribution.patch" | "contribution.remove" => {
            let id = text(input, "contribution_id")?;
            let contribution = array_mut(document, "contributions")?
                .iter_mut()
                .find(|c| c["id"] == id)
                .ok_or_else(|| invalid("ContributionId is not in this document"))?;
            if !human
                && (contribution["author_ref"] != author.principal_ref
                    || contribution["locked"] == true
                    || contribution["human_touched"] == true)
            {
                return Err(denied("Agent may edit only its own unlocked, untouched contribution; review does not transfer human authorship"));
            }
            if contribution["removed"] == true {
                return Err(conflict("contribution is already removed"));
            }
            if input["operation"] == "contribution.remove" {
                contribution["removed"] = json!(true);
                contribution["html"] = json!("");
            } else {
                contribution["html"] = json!(clean(
                    input
                        .get("html")
                        .and_then(Value::as_str)
                        .ok_or_else(|| invalid("patch html required"))?
                )?);
            }
            if human {
                contribution["human_touched"] = json!(true);
                contribution["locked"] = json!(true);
            }
            contribution["edited_by"] = json!(author.principal_ref);
            contribution["edited_at_unix_seconds"] = json!(now);
        }
        "field.set" => {
            if !human {
                return Err(denied(
                    "changing the human field value is not an Agent contribution",
                ));
            }
            let id = text(input, "field_id")?;
            let field = document["fields"]
                ...
                .ok_or_else(|| invalid("field_id not found"))?;
            let pointer = text(field, "template_pointer")?.to_owned();
            let destination = document["template_payload"]
                .pointer_mut(&pointer)
                .ok_or_else(|| invalid("actual template field no longer exists"))?;
            *destination = input
                .get("value")
                .cloned()
                .ok_or_else(|| invalid("field value required"))?;
        }
        "title.set" | "summary.set" | "lifecycle.set" => {
            if !human {
                return Err(denied(
                    "human document metadata requires authenticated human mutation",
                ));
            }
            let key = match input["operation"].as_str().unwrap() {
                "title.set" => "title",
                "summary.set" => "summary",
                _ => "lifecycle",
            };
            let value = input
                .get("value")
                .and_then(Value::as_str)
                .ok_or_else(|| invalid("metadata value must be text"))?;
            if value.len() > 65536 {
                return Err(invalid("metadata value exceeds bounded source field size"));
            }
            if key == "lifecycle" && !matches!(value, "open" | "closed") {
                return Err(invalid("document lifecycle is open or closed"));
            }
...
fn intent_path(key: &str) -> String {
    format!("{INTENTS}/{}.json", source::key(key))
}
...
fn finish(scope: &Scope, intent: &mut Intent) -> io::Result<Value> {
    ...
    if current.source.path != intent.source_path {
        return Err(conflict(
            "document moved after mutation intent; explicit source recovery required",
        ));
    }
    ...
    if committed.revision.revision != intent.next_revision {
        return Err(conflict(
            "native document commit did not match its intended revision",
        ));
    }
    retain_metadata(
        scope,
...
pub fn mutate(scope: &Scope, input: &Value, principal: &Principal, now: u64) -> io::Result<Value> {
    mutate_as(
        scope,
        input,
        &ContributionAuthor::from(principal),
        None,
        now,
    )
}
pub(crate) fn mutate_reviewed(
    scope: &Scope,
    input: &Value,
    author: &ContributionAuthor,
    reviewer: &Principal,
...
fn mutate_as(
    ...
    match crate::source_safety::read(&scope.root, &intent_path(&request_key)) {
        Ok(raw) => {
            ...
            if intent.schema != "central.document-mutation/v1"
                ...
                || intent.source_ref != reference
                || intent.document_id != id
                || intent.request_key != request_key
                || intent.digest != digest
                || intent.actor.principal_ref != author.principal_ref
                || intent.reviewer_ref != reviewer.map(|p| p.principal_ref.clone())
            {
                return Err(conflict(
    ...
    let mut intent = Intent {
        schema: "central.document-mutation/v1".into(),
