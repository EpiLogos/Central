...
pub(crate) fn text<'a>(input: &'a Value, field: &str) -> io::Result<&'a str> {
    input
        ...
        .ok_or_else(|| {
            invalid(format!(
                "{field} requires non-empty text of at most 4096 bytes"
            ))
        })
}
...
impl Scope {
    ...
    pub fn now_dir(&self) -> String {
        format!("{}/agents/now/clearings", self.prefix)
    }
    pub fn day_dir(&self) -> String {
        format!("{}/user/day", self.prefix)
    }
    ...
    pub fn relations(&self) -> io::Result<(Value, String)> {
        let raw = match crate::source_safety::read(&self.root, &self.relations_path) {
            ...
            Err(e) if e.kind() == io::ErrorKind::NotFound => {
                return Ok((
                    json!({"schema":self.relations_schema,"project_id":self.relations_id,"relations":[]}),
                    "absent".into(),
                ));
            }
            Err(e) => return Err(e),
    ...
    pub fn reconcile(
        &self,
        actor: Option<(&str, &str, Option<String>)>,
        refs: &[String],
    ) -> io::Result<Value> {
        ...
        Ok(
            json!({"world_ref":report.horizon.world_ref,"cursor":report.horizon.cursor,"changes":report.new_changes}),
        )
    }
    ...
    pub fn bind(&self, binding: &SourceBinding, recorded_at: u64) -> io::Result<()> {
        ...
        for old in entries.iter() {
            if old["ref"] == binding.source_ref || old["path"] == binding.path {
                ...
                return Err(conflict(
                    "SourceRef/path is already bound; use migration rather than replacing identity",
                ));
            }
        }
        entries.push(json!({
        ...
        if basis == "absent" {
            put_new(&self.root, &self.relations_path, &content)?;
        } else {
            crate::source_safety::replace(&self.root, &self.relations_path, &basis, &content)?;
        }
        Ok(())
    }
    pub fn create_agent_source(
        ...
        role: &str,
        at: u64,
    ) -> io::Result<SourceReading> {
        if !Path::new(path).starts_with(format!("{}/agents", self.prefix)) {
            return Err(denied(
                "Agent source creation must remain in this register's agents aperture",
            ));
        }
        ...
        let binding = SourceBinding {
            source_ref: self.source_ref(path),
            path: path.into(),
            roles: vec![role.into()],
...
pub(crate) fn directories(root: &Path, relative: &Path) -> io::Result<File> {
    ...
    for component in relative.components() {
        let std::path::Component::Normal(part) = component else {
            return Err(invalid("invalid directory member"));
        };
...
pub(crate) fn put_new(root: &Path, relative: &str, content: &str) -> io::Result<bool> {
    ...
    let parent_path = path
        .parent()
        .ok_or_else(|| invalid("source parent required"))?;
    let parent = directories(root, parent_path)?;
    let name = path
        .file_name()
        .and_then(|s| s.to_str())
        .ok_or_else(|| invalid("UTF-8 source name required"))?;
    ...
    match crate::source_safety::read(root, relative) {
        Ok(existing) => {
            if existing != content {
                return Err(conflict(
                    "existing source bytes differ; no overwrite performed",
                ));
            }
            if let Ok(stage) = crate::file_mutation::open_native_file(root, &staging_path) {
                ...
                if stage.metadata()?.ino() == current.metadata()?.ino()
                    && stage.metadata()?.dev() == current.metadata()?.dev()
                {
    ...
    if held.dev() != current.dev() || held.ino() != current.ino() {
        return Err(conflict(
            "destination parent changed before source publication",
        ));
    }
