...
fn inspect_action(
    ...
    match inspect_current_machine(context, "machine.inspect", &MachineInspectionInput::default()) {
...
fn inspection_input(declaration: &MachineDeclaration) -> MachineInspectionInput {
    MachineInspectionInput {
        package_ids: declaration.requirements.packages.iter().map(|item| item.id.clone()).collect(),
        configuration_ids: declaration.requirements.configurations.iter().map(|item| item.id.clone()).collect(),
        service_ids: declaration.requirements.services.iter().map(|item| item.id.clone()).collect(),
    }
}
