...
fn document_text(scope: &Scope) -> io::Result<Option<String>> {
    let p = safe_member(&scope.root, scope.relations_path(), false)?;
    if p.exists() {
...
pub(crate) fn apply(
    ...
    if input["quiesced"] != true {
        return Err(invalid("A move requires explicit quiesced=true: caller has stopped affected sessions/processes"));
    }
    let id = text(input, "plan_id")?;
    safe_member(root, ".central/bkmr/moves", true)?;
    let mut plan: Plan =
    ...
    let _source_lock = crate::source_safety::lock(&owner.root, "source-mutation.lock")?;
    ...
    for doc in &plan.documents {
        let current = document_text(scope(all, &doc.world)?)?;
    ...
    for doc in &plan.documents {
        ...
        let target = safe_member(&world.root, world.relations_path(), false)?;
        if rollback {
            ...
            safe_directory(
                &world.root,
                Path::new(world.relations_path()).parent().unwrap(),
            )?;
