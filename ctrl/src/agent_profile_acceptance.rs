...
struct Input {
    ...
    #[serde(default)]
    expected_authority_revision: Option<String>,
}
...
fn digest(text: &str) -> String {
    format!("sha256:{}", source::key(text))
}
fn receipt_path(profile_ref: &str, content_digest: &str) -> String {
    format!(
        ".central/agent-profile-acceptances/{}-{}.json",
        source::key(profile_ref),
        source::key(content_digest)
    )
}
fn profile_source(
    store: &AgentProfileStore,
    profile_ref: &str,
) -> io::Result<(crate::agent_profile_store::AgentProfileReading, String)> {
    let reading = store
        .read(profile_ref)
    ...
    if profile != reading.profile {
        return Err(conflict(
            "AgentProfile changed while being read; review the new source",
        ));
    }
    if !crate::source_horizon::retrieval_allowed(
        store.owner_root(),
        &store.owner_root().join(&reading.source_path),
    ) {
        return Err(io::Error::new(
            io::ErrorKind::PermissionDenied,
            "AgentProfile source is withheld from Agent retrieval",
        ));
    }
...
fn review(store: &AgentProfileStore, scope: &Scope, profile_ref: &str) -> io::Result<Value> {
    ...
    if let Some(a) = &acceptance {
        if a.schema != SCHEMA
            || a.profile_ref != profile_ref
            || a.agent_ref != reading.profile.agent_ref
            || a.profile_revision != reading.profile.revision
            || a.content_digest != content_digest
            || a.scope_ref != scope.world_ref
            || a.principal_ref.is_empty()
            || a.authority_ref.is_empty()
            || a.authority_revision.is_empty()
            || a.acceptance_ref != format!("agent-profile-acceptance:{}", source::key(&path))
        {
            return Err(invalid(
                "Native AgentProfile acceptance is not bound to this exact source",
            ));
        }
    }
    Ok(
        json!({"schema":"central.agent-profile-review/v1", "profile":reading.profile,
...
pub fn execute_with_token_at(
    ...
    let project = if input.scope == "project" {
        Some(
            input
                ...
                .filter(|s| !s.is_empty())
                .ok_or_else(|| {
                    invalid("Project scope requires the existing Central Work member")
                })?,
        )
    } else {
        if input.project.is_some() {
            return Err(invalid(
                "Root Agent definition must not name a child Project",
            ));
        }
        None
    };
    ...
    let store = if project.is_some() {
        AgentProfileStore::project(&scope.root)
    } else {
        AgentProfileStore::personal(&scope.root)
    };
    if action == ROSTER {
        if input.profile_ref.is_some() {
            return Err(invalid("Roster does not accept a selected profile"));
        }
        ...
        return Ok(
            json!({"schema":"central.agent-profile-roster/v1", "scope_ref":scope.world_ref,"profiles":rows,"execution_authority_granted":false}),
        );
    }
    let profile_ref = input
        .profile_ref
        .as_deref()
        .filter(|s| !s.is_empty())
        .ok_or_else(|| invalid("profile_ref is required"))?;
    if action == REVIEW {
        return review(&store, &scope, profile_ref);
    }
    if action != ACCEPT {
        return Err(invalid("Unknown AgentProfile acceptance operation"));
    }
    ...
    let _profile = store
        .mutation_lock(profile_ref)
        .map_err(|e| invalid(e.to_string()))?;
    let principal = authority::authenticate(
        &scope,
        token,
        ACCEPT,
        input.expected_authority_revision.as_deref(),
        now,
    )?;
    ...
    let content_digest = current["content_digest"]
        .as_str()
        .ok_or_else(|| invalid("Source digest unavailable"))?;
    let path = receipt_path(profile_ref, content_digest);
    let acceptance = Acceptance {
        schema: SCHEMA.into(),
        acceptance_ref: format!("agent-profile-acceptance:{}", source::key(&path)),
        profile_ref: profile_ref.into(),
        agent_ref: current["profile"]["agent_ref"]
            .as_str()
            .ok_or_else(|| invalid("Agent identity unavailable"))?
            .into(),
        profile_revision: current["profile"]["revision"]
            .as_str()
            .ok_or_else(|| invalid("Profile revision unavailable"))?
            .into(),
        content_digest: content_digest.into(),
        scope_ref: scope.world_ref.clone(),
        principal_ref: principal.principal_ref,
        authority_ref: principal.authority_ref,
        ...
        Err(error) => {
            let (status, code) = match error.kind() {
                io::ErrorKind::PermissionDenied => (
                    ...
                    "agent_profile_acceptance_denied",
                ),
                io::ErrorKind::AlreadyExists => (
                    ResultStatus::VerificationFailure,
                    "agent_profile_review_stale",
                ),
                _ => (
                    ResultStatus::InvalidInput,
                    "agent_profile_acceptance_invalid",
                ),
            };
            ActionResult::failure_coded(
                ...
                error.to_string(),
                Some(
                    json!({"setup":{"owner":"central","section":"native-authority","action":ACCEPT,"credential_channel":"native-protected-process","raw_credentials_accepted":false},"automatic_retry":false}),
                ),
            )
        }
    }
...
fn accept_action(
    ...
    run(ACCEPT, input, context)
}
pub fn register(registry: &mut ActionRegistry) {
    for (action, title, mutating, handler) in [
        (
            REVIEW,
            "Review exact Agent definition",
            false,
            review_action as crate::action::ActionHandler,
        ),
        (
            ROSTER,
            "Read native reusable Agent roster",
            false,
            roster_action as crate::action::ActionHandler,
        ),
        (
            ACCEPT,
            "Accept reviewed reusable Agent definition",
            true,
            accept_action as crate::action::ActionHandler,
        ),
    ] {
        let mut inputs = vec![("scope", true), ("project", false)];
        if action != ROSTER {
            inputs.push(("profile_ref", true));
        }
        if mutating {
            inputs.extend([
                ("expected_revision", true),
                ("expected_content_digest", true),
