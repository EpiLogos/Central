...
pub mod conformance {
    ...
    use super::{ConnectorMetadata, MutationClass, WorkDiscovery, WORK_DISCOVERY_CONTRACT_ID, WORK_DISCOVERY_PORT_ID};
    ...
    pub fn work_discovery<C: WorkDiscovery>(metadata: &ConnectorMetadata, connector: &C, fixture_root: &Path) -> ConformanceReport {
        ...
        if !metadata.implemented_ports.iter().any(|port| port.id == WORK_DISCOVERY_PORT_ID && port.contract == WORK_DISCOVERY_CONTRACT_ID) {
            failures.push(format!("manifest must declare {WORK_DISCOVERY_CONTRACT_ID}"));
        }
    ...
    pub fn metadata_failures(metadata: &ConnectorMetadata) -> Vec<String> {
        ...
        if metadata.display_name.trim().is_empty() { failures.push("connector display name must not be empty".into()); }
        if metadata.implemented_ports.is_empty() { failures.push("connector must declare an implemented Port".into()); }
        if metadata.supported_environments.is_empty() { failures.push("connector must declare a supported environment".into()); }
