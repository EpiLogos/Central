...
fn create_ordinary(root: &Path, input: &Value) -> io::Result<Value> {
    ...
    let object = input
        .as_object()
        .ok_or_else(|| invalid("Creation input must be an object"))?;
    ...
    let parent_loc: CentralPathRef =
        serde_json::from_value(input.get("parent").cloned().unwrap_or(Value::Null))?;
    let parent_path = parent_loc.resolve(&root)?;
    if !parent_path.is_dir() {
        return Err(invalid(
            "The selected parent must already be a native directory",
        ));
    }
    ...
    let relative = if parent_loc.path.is_empty() {
        name.into()
    } else {
        format!("{}/{name}", parent_loc.path)
    };
    ...
    if record.exists() {
        ...
        if let Ok(reading) = read_file(&root, &loc, crate::files::FileEncoding::Utf8) {
            ...
            if pending.exists() {
                ...
                if event.revision != revision || !event.previous_revision.is_empty() {
                    return Err(invalid("Interrupted creation has another history basis"));
                }
                fs::rename(&pending, area.join(format!("event-{}.json", event.cursor)))?;
                File::open(&area)?.sync_all()?;
            }
            return Ok(
                json!({"schema":"central.file-mutation/v1","outcome":"unchanged","location":loc,"revision":revision,"changed":false,"operation_ref":operation,"current":reading,"automatic_agent_or_model_invocation":false}),
            );
        }
        ...
        return Err(io::Error::other(
            "An interrupted file mutation is unresolved; do not create over its recovery state",
        ));
    }
    match fs::symlink_metadata(root.join(&loc.path)) {
        Ok(_) => {
            return Err(conflict(
                "The native filename already exists; creation never overwrites",
            ))
        }
    ...
    let result = (|| {
        ...
        Ok(
            json!({"schema":"central.file-mutation/v1","outcome":"created","location":loc,"revision":revision,"changed":true,"operation_ref":operation,"change":event,"current":reading,"automatic_agent_or_model_invocation":false}),
        )
    })();
    unsafe {
