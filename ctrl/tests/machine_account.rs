...
fn identity_is_stable_and_persisted_under_derived_local_state() {
    ...
    let first_id = first.data.unwrap()["current"]["machine_id"]
        .as_str()
        .unwrap()
        .to_owned();
    let second_id = second.data.unwrap()["current"]["machine_id"]
        .as_str()
        .unwrap()
        .to_owned();
    assert_eq!(
        first_id, second_id,
        "machine identity must be stable across calls"
    );

    let identity_file = root.join(".central/machines/identity.json");
    assert!(
        identity_file.is_file(),
        "identity must persist under .central derived state"
    );
...
fn account_composes_observed_and_authored_without_writing_control() {
    ...
    assert_eq!(
        fs::read_to_string(&authored_path).unwrap(),
        authored_before,
        "observation must never author into Control"
    );
    ...
    assert_eq!(
        data["last_observation"]["connector_id"],
        json!("reference.machine-static")
    );
...
fn drift_and_reconciliation_are_derived_from_observation() {
    ...
    write_role(
        &root,
        "primary-workstation",
        &["remote-shell", "media-acceleration"],
    );
    ...
    let missing = drift
        .iter()
        .find(|entry| entry["id"] == "remote-shell")
        .unwrap();
...
fn observation_record_persists_under_central_for_stale_fallback() {
    ...
    let result = execute_account(&connectors, &root);
    assert_eq!(result.status, ResultStatus::Success);
    let machine_id = result.data.as_ref().unwrap()["current"]["machine_id"]
        .as_str()
        .unwrap()
        .to_owned();
    let record_path = root
        .join(".central/machines/observed")
        .join(format!("{machine_id}.json"));
    assert!(
        record_path.is_file(),
        "observation must persist under .central derived state"
    );
    ...
    assert_eq!(
        data["current"]["machine_id"],
        json!(machine_id),
        "machine remains recognisable from cache"
    );
}
