use central_ctrl::{
    create_core_action_registry, initialize_central, run_guided_action_picker,
    ActionExecutionContext, CapabilityProbe, Connector, ConnectorContext, ConnectorManifest,
    ConnectorPortDeclaration, ConnectorRegistry, FilesystemWorkConnector, NativeOpen,
    NativeOpenInput, NativeOpenOutput, PortContract, PortError, ResultStatus, RootOptions,
    TerminalSurface, CONNECTOR_API_VERSION, NATIVE_OPEN_PORT,
};
use serde_json::json;
...
impl Connector for PickerNativeConnector {
    ...
    fn probe(&self, port: &PortContract, context: &ConnectorContext) -> CapabilityProbe {
        if context.platform == "test"
            && port.id == NATIVE_OPEN_PORT.id
            && port.version == NATIVE_OPEN_PORT.version
        {
            CapabilityProbe::available()
        } else {
            CapabilityProbe::unavailable(
                "Picker native Connector is not eligible for this request.",
            )
        }
    }
...
fn guided_project_entry_searches_registry_confirms_mutation_and_executes_the_same_work_open_action()
{
    ...
    assert!(surface
        .output
        .iter()
        .any(|line| line.contains("Matching Actions")));
    assert!(surface
        .output
        .iter()
        .any(|line| line.contains("Choices for query")));
    assert!(surface
        .output
        .iter()
        .any(|line| line.contains("externally-mutating")));
}
...
fn back_from_selectable_value_returns_to_action_search_without_executing_it() {
    ...
    let ctx = context(&root, &connectors, &connector_context, &mut options);
    let mut surface =
        ScriptedSurface::new(&["work.open", "1", "/back", "work.search", "1", "project-a"]);
    let result = run_guided_action_picker(&registry, &ctx, &mut surface);
