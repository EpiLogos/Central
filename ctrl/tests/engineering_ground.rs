...
fn render_errors_not_found_when_no_statements_exist() {
    ...
    let error = render_engineering_ground(temp.path()).unwrap_err();
...
fn render_skips_a_missing_statement_and_stays_well_formed() {
    ...
    // Present statements render their sections.
    assert!(rendered.contains("You branch small and commit honestly."));
    assert!(rendered.contains("You change the smallest thing that can work."));
    assert!(rendered.contains("You return with evidence of done."));
    // The missing statement is skipped, not errored, and leaves no trace.
    assert!(!rendered.contains("base-skillset"));
    assert!(!rendered.contains("You reach for the essentials first."));
    // The render is otherwise well-formed: derived provenance header is intact
    // and only lists the three present statements.
    assert!(rendered.contains("provenance: generated-derived"));
    assert!(rendered.contains(
        "central:source:control:root:Control/agents/governance/engineering/agent-operations.md"
    ));
