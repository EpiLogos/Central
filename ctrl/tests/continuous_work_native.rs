...
fn world() -> World {
    ...
    let policies = [
        ...
        (
            "time.json",
            "civil-time-policy",
            json!({"schema":"central.civil-time-policy/v1","scope_ref":"control:root","timezone":"Europe/London","day_boundary_minutes":240,"automatic_day_rollover":true}),
        ),
        (
            "authority.json",
...
fn civil_day_rollover_does_not_close_writing_rewrite_tasks_or_clear_now() {
    ...
    let original_now =
        fs::read(world.path().join(now["source"]["path"].as_str().unwrap())).unwrap();
    ...
    for key in [
        "prior_writing_closed",
        "open_editor_changed",
        "tasks_carried_or_ticked",
        "now_cleared_or_archived",
        "automatic_agent_or_model_invocation",
    ] {
        assert_eq!(next[key], false, "{key}");
    }
...
fn daylight_saving_repeated_hour_and_project_days_use_the_same_root_policy() {
    ...
    for instant in ["2026-10-25T00:30:00Z", "2026-10-25T01:30:00Z"] {
        ...
        for project in ["one", "two"] {
            ...
            assert!(day["source"]["path"]
                .as_str()
                .unwrap()
                .starts_with("ProjectCentral/user/day/"));
        }
    }
}
#[test]
fn human_label_does_not_authenticate_day_closure_and_agent_cannot_close_it() {
    ...
    assert_eq!(
        execute_at(world.path(), "day_lifecycle", &input, 100)
            .unwrap_err()
            .kind(),
        io::ErrorKind::PermissionDenied
    );
    assert_eq!(
        run(&world, "day_lifecycle", &input, AGENT, 100)
            .unwrap_err()
            .kind(),
        io::ErrorKind::PermissionDenied
    );
    ...
    assert_eq!(
        run(&world, "day_lifecycle", &input, HUMAN, 100)
            .unwrap_err()
            .kind(),
...
fn stale_time_policy_and_changed_authority_fail_without_source_effects() {
    let world = world();
    let time = execute_at(world.path(), "time_policy", &json!({}), 100).unwrap();
...
fn now_archive_reentry_keeps_identity_artifacts_and_native_history() {
    let world = world();
    let allocation = allocate(&world, None, 100);
    let artifact =
        Path::new(allocation["writable_destination"].as_str().unwrap()).join("retained.md");
    ...
    let history = execute_at(
        world.path(),
        "source_history",
        &json!({"source_ref":allocation["source"]["ref"]}),
        103,
    )
    .unwrap();
    ...
    assert_eq!(
        history["entries"][0]["previous_revision"],
        archived["revision"]["revision"]
    );
}
#[test]
fn pending_receiving_survives_closure_and_blocks_now_archive() {
    ...
    assert!(world
        .path()
        .join(".central/source-returns/contributions/pending.json")
        .exists());
}
#[test]
fn real_cli_binary_roundtrips_policy_allocation_source_and_rejection() {
    ...
    let first = invoke("central.now.allocate", request.clone());
    assert_eq!(first["ok"], true, "{first}");
    let second = invoke("central.now.allocate", request);
    assert_eq!(second["ok"], true);
    assert_eq!(first["data"]["now_ref"], second["data"]["now_ref"]);
    assert_eq!(first["data"]["revision"], second["data"]["revision"]);
    let read = invoke(
        "central.now.read",
        json!({"now_ref":first["data"]["now_ref"]}),
    );
    assert_eq!(read["data"]["revision"], first["data"]["revision"]);
    for (destination, allowed) in [
        ("Work/one/src/main.rs", true),
        ("Work/root-scratch.diff", false),
    ] {
        let response = invoke(
            "central.work.validate",
            json!({"destination":destination,"now_ref":first["data"]["now_ref"],"expected_now_revision":first["data"]["revision"]["revision"],"expected_policy_revision":policy["data"]["revision"]}),
        );
