...
fn two_valid_reference_connectors_resolve_stably_independent_of_registration_order() {
    ...
    assert_eq!(
        a.diagnostics
            ...
            .collect::<Vec<_>>(),
        vec!["reference.work-filesystem", "reference.work-static",]
    );
    assert_eq!(
        a.diagnostics.selected_connector.as_ref().unwrap().id,
        "reference.work-filesystem"
    );
    assert_eq!(
        b.diagnostics.selected_connector.as_ref().unwrap().id,
        "reference.work-filesystem"
    );
}
...
fn work_list_action_depends_on_port_and_reports_selected_connector() {
    ...
    fs::write(
        root.join("Work").join("ordinary-file.txt"),
...
fn cli_exposes_the_same_action_and_connector_diagnostics() {
    ...
    let environment = central_ctrl::CliEnvironment {
    ...
    assert_eq!(
        value["data"]["diagnostics"]["selected_connector"]["id"],
        "reference.work-filesystem"
    );
...
fn static_reference_connector_uses_the_same_typed_port_contract() {
    ...
    connectors
        .register(StaticWorkConnector::new(vec![item]))
        .unwrap();
    let context = ConnectorContext {
        platform: "linux".to_owned(),
    };
    ...
    let output = implementation
        .list(&central_ctrl::WorkDiscoveryInput {
