use central_ctrl::reference_connectors::{
    FilesystemWorkDiscovery, StaticWorkDiscovery, filesystem_work_discovery_metadata,
    static_work_discovery_metadata,
};
use central_ctrl::sdk::{PortCompatibility, WORK_DISCOVERY_PORT_ID, WorkItem, conformance};
...
fn both_reference_connectors_pass_public_conformance() {
    ...
    let first = conformance::work_discovery(
        &filesystem_work_discovery_metadata(),
        &FilesystemWorkDiscovery,
        &root,
    );
    assert!(first.passed, "{:?}", first.failures);
    let second_connector = StaticWorkDiscovery::new(vec![WorkItem {
        name: "alpha".into(),
        path: root.join("Work/alpha").display().to_string(),
    }]);
    let second =
        conformance::work_discovery(&static_work_discovery_metadata(), &second_connector, &root);
...
fn incompatible_contract_fails_conformance() {
    ...
    metadata.implemented_ports = vec![PortCompatibility {
        id: WORK_DISCOVERY_PORT_ID.into(),
        contract: "WorkDiscovery/v0".into(),
    }];
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|failure| failure.contains("WorkDiscovery/v1"))
    );
}
...
fn metadata_validation_is_reusable() {
    ...
    assert!(
        failures
            .iter()
            .any(|failure| failure.contains("entrypoint"))
    );
}
