...
test("root discovery prefers explicit override, then CENTRAL_ROOT, then HOME/Central", async () => {
  ...
  assert.deepEqual(resolveCentralRoot({ explicitRoot: explicit, env: { CENTRAL_ROOT: configured }, home }), {
    path: explicit,
    source: "explicit",
  });
  assert.deepEqual(resolveCentralRoot({ env: { CENTRAL_ROOT: configured }, home }), {
    path: configured,
    source: "environment",
  });
  assert.deepEqual(resolveCentralRoot({ env: {}, home }), {
    path: join(home, "Central"),
    source: "default",
  });
});
...
test("initialization creates only the required roots and is safe to repeat", async () => {
  ...
  for (const controlRoot of ["user", "agents", "machines"]) {
    assert.deepEqual(await readdir(join(root, "Control", controlRoot)), []);
  }
});
...
test("init reports an existing non-directory root as invalid Central structure", async () => {
  ...
  const registry = createCoreActionRegistry();
  const result = await registry.execute("central.init", {}, { rootOptions: { explicitRoot: root, env: {} } });
...
test("registry converts unexpected executor exceptions into structured internal failures", async () => {
  ...
  registry.register({
    id: "test.fail",
    title: "Fail",
    description: "Test failure conversion.",
    inputs: [],
    output: { type: "test" },
...
test("ActionRegistry can register independent Actions without changing registry internals", async () => {
  ...
  registry.register({
    id: "example.read",
    title: "Example",
    description: "Example extension point for later core Actions.",
    inputs: [],
    output: { type: "example" },
    mutationClass: "read-only",
    previewSupported: false,
...
test("first Port slice resolves two valid reference Connectors deterministically", async () => {
  ...
  } = await import("../../connectors/reference/work-discovery.js");

  const first = new ConnectorRegistry()
  ...
  assert.deepEqual(a.diagnostics.eligible.map((item) => item.id), [
...
test("work.list depends on WorkDiscovery and reports selected Connector diagnostics", async () => {
  ...
  const connectors = new ConnectorRegistry().register(createStaticWorkDiscoveryConnector([
    { name: "beta", path: join(root, "Work", "beta") },
    { name: "alpha", path: join(root, "Work", "alpha") },
  ]));
  const registry = createCoreActionRegistry();
  assert.deepEqual(registry.get("work.list").requiredPorts, ["WorkDiscovery"]);
  const result = await registry.execute("work.list", {}, {
