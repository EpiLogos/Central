use central_connector_sdk::{AutomationRunInput, ConnectorContext, ConnectorRegistry, ConnectorSummary, AUTOMATION_PORT};
...
use central_ctrl::{
    create_core_action_registry, run_cli_with_runtime, ActionAvailability, ActionDescriptor,
    ActionExecutionContext, ActionInputDefinition, ActionOutputDefinition, ActionRegistry,
    ActionResult, CliEnvironment, CliExecution, MutationClass, ResultStatus, RootOptions,
    TerminalSurface,
};
...
fn automation_run_action(
    ...
    let Some(provider) = connector.automation() else {
        return ActionResult::failure(
            Some("automation.run"),
    ...
    match provider.run(&AutomationRunInput {
